...
class Cutout(object):
    ...
    def __call__(self, img):
        ...
        # w = img.size(1)
        h, w, c = img.shape
        mmask = np.ones((h, w, c), np.float32)
        ...
        # mmask = torch.from_numpy(mmask)
        # mmask = mmask.expand_as(img)
        img = img * mmask
...
class Data(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.cfg.mode=='train':
            try:
                ...
                print("#######"+str(name))
            image1, mask1 = self.normalize(image, mask)
            image1, mask1 = self.randomcrop(image1, mask1)
            image1, mask1 = self.randomflip(image1, mask1)
            # image2 = self.normalize(image)
            # image2 = self.randomcrop(image2)
            # image2 = self.randomflip(image2)
            # image2 = self.randomrotate(image2)

            image2 = self.colorenhance(image1)
            image2 = self.Cutout(image2)

            return image1, mask1, image2
    ...
    def collate(self, batch):
        ...
        for i in range(len(batch)):
            ...
            image2[i] = cv2.resize(image2[i], dsize=(size, size), interpolation=cv2.INTER_LINEAR)

        image1  = torch.from_numpy(np.stack(image1, axis=0)).permute(0,3,1,2)
        mask   = torch.from_numpy(np.stack(mask, axis=0)).unsqueeze(1)
        image2 = torch.from_numpy(np.stack(image2, axis=0)).permute(0, 3, 1, 2)
        return image1, mask, image2
